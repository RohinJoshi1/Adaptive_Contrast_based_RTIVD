...
class Dehazer:
    ...
    def TransmissionEstimation(self, blk_size, alpha):
...
def dehaze_video(video_url):
    ...
    dhz = Dehazer(init)
    dhz.AirLightEstimation((0,0), init.shape[0], init.shape[1])
    ...
    while True:
        ...
        if frame.any():         
            ...
            if frame_diff > 0.6:
                dhz.TransmissionEstimation(blk_size, alpha=0.9)
